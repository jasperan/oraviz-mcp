...
User-facing story: `README.md`. Full study with the token benchmark against the
official SQLcl MCP server: `paper/paper.pdf` (LaTeX sources in `paper/`).
Measurements: `benchmarks/`. Results deck: `slides/`. Showcase site: `docs/`
(GitHub Pages).
...
# Paper, figures, deck, site
uv run python paper/make_figures.py                              # PDF + PNG figures from benchmark JSON
cd paper && ~/opt/tectonic/tectonic paper.tex                    # this machine has Tectonic, not pdflatex
uv run --with "python-pptx>=1.0" python slides/build_deck.py     # rebuilds the deck from the PNG figures
...
- README claims must match behavior; if you change a guarantee, update README,
  CHANGELOG, and the paper/benchmark text that cites it.
...
- No new runtime dependencies unless justified; benchmark tooling (tiktoken)
  belongs to the dev extra, one-off tooling (python-pptx) runs via
  `uv run --with`.
