...
Table~\ref{tab:totals} aggregates the workflow. Tool schemas cost $59.7$\% fewer tokens for OraViz, tool results cost $53.3$\% fewer, and the workflow total is $53.3$\% lower, $2{,}337$ tokens against $5{,}006$. Figure~\ref{fig:benchmark-totals} visualizes the budget by stage, and the SQLcl-only connect step adds two tokens that the total accounts for. Under \texttt{o200k\_base} the numbers move by less than one point ($53.2$\% total savings), so the aggregate result is not an artifact of one tokenizer vocabulary.
...
\midrule
Tool schemas (once per session) & 862 & 2{,}139 & 59.7\% \\
Tool results (five questions) & 2{,}337 & 5{,}004 & 53.3\% \\
Workflow total & 2{,}337 & 5{,}006 & 53.3\% \\
\bottomrule
\end{tabular}
